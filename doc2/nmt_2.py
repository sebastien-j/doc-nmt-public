...
def build_sampler(tparams, options, trng, use_noise=None):
    ...
    if options['decoder'].startswith('lstm'):
        outs = [init_state, ctx, ctx_c, init_memory]
    else:
        outs = [init_state, ctx, ctx_c]
    ins = [x, xc]
    ...
    if options['decoder'].startswith('lstm'):
        inps = [y, ctx, init_state, ctx_c, init_memory]
        outs = [next_probs, next_sample, next_state, next_memory]
    else:
        inps = [y, ctx, init_state, ctx_c]
...
def build_sampler_2(tparams, options, trng, use_noise=None):
    ...
    print 'Building f_next..',
    if options['decoder'].startswith('lstm'):
        inps = [y, ctx, init_state, ctx_c, init_memory, x_mask, xc_mask]
        outs = [next_probs, next_sample, next_state, next_memory]
    else:
        inps = [y, ctx, init_state, ctx_c, x_mask]
        outs = [next_probs, next_sample, next_state]
...
def gen_sample_2(tparams, f_init_2, f_next_2, x, xc, x_mask, xc_mask, options, trng=None, maxlen=30):
    ...
    for ii in xrange(maxlen):
        if options['decoder'].startswith('lstm'):
            inps = [next_w, ctx, next_state, sc, next_memory, x_mask, xc_mask]
        else:
            inps = [next_w, ctx, next_state, sc, x_mask, xc_mask]
        ret = f_next_2(*inps)
        if options['decoder'].startswith('lstm'):
            next_p, next_w, next_state, next_memory = ret[0], ret[1], ret[2], ret[3]
        else:
