...
from data_iterator_ctx_only import TextIterator
...
# batch preparation
def prepare_data(seqs_y, seqs_xc, maxlen=None, n_words_src=30000,
                 n_words=30000):
    ...
    if maxlen is not None:
        ...
        new_lengths_xc = []
        new_num_words = []
        for s_y, s_xc, l_y, l_xc, l_nw in zip(seqs_y, seqs_xc, lengths_y, lengths_xc, num_words):
            if l_y < maxlen:
    ...
    if len(lengths_y) < 1:
        return None, None, None, None, lengths_y, lengths_xc, xc_mask_2, xc_mask_3

    n_samples = len(seqs_xc)
    ...
    for idx, [s_y, s_xc] in enumerate(zip(seqs_y, seqs_xc)):
    ...
    return y, y_mask, xc, xc_mask, lengths_y, lengths_xc, xc_mask_2, xc_mask_3
...
def init_params(options):
    ...
    # embedding
    if not options['kwargs'].get('tc', False):
...
def build_model(tparams, options):
    ...
    proj = get_layer(options['encoder'])[1](tparams, context_emb, options,
                                            prefix='encoder',
                                            mask=xc_mask_2)

    projr = get_layer(options['encoder'])[1](tparams, context_emb[::-1], options,
                                             prefix='encoder_r',
                                             mask=xc_mask_2[::-1])
    ...
    ctx_mean = (ctx * xc_mask_2[:, :, None]).sum(0) / xc_mask_2.sum(0)[:, None]
    ...
    return trng, use_noise, y, y_mask, xc, xc_mask, opt_ret, cost, xc_mask_2, xc_mask_3
...
def build_sampler(tparams, options, trng, use_noise=None):
    ...
    if options['decoder'].startswith('lstm'):
        ...
        outs = [init_state, ctx]
    if options['kwargs'].get('context_birnn', False):
        ins = [xc, xc_mask, xc_mask_2, xc_mask_3]
    else:
        ins = [xc, xc_mask, xc_mask_3]
...
def gen_sample(tparams, f_init, f_next, xc, xc_mask, xc_mask_2, xc_mask_3, options, trng=None, k=1, maxlen=30,
    ...
    if options['kwargs'].get('context_birnn', False):
        ret = f_init(xc, xc_mask, xc_mask_2, xc_mask_3)
    else:
        ret = f_init(xc, xc_mask, xc_mask_3)
...
def build_sampler_2(tparams, options, trng, use_noise=None):
    ...
    # encoder
    proj = get_layer(options['encoder'])[1](tparams, context_emb, options,
                                            prefix='encoder', mask=xc_mask_2)
    projr = get_layer(options['encoder'])[1](tparams, context_emb[::-1], options,
                                             prefix='encoder_r', mask=xc_mask_2[::-1])
    ...
    proj = get_layer(options['decoder'])[1](tparams, emb, options,
                                            prefix='decoder',
                                            mask=None, context=ctx, context_mask=xc_mask_2,
                                            one_step=True,
                                            init_state=init_state,
                                            init_memory=init_memory)
    # get the next hidden state
    ...
    print 'Building f_next..',
    if options['decoder'].startswith('lstm'):
        inps = [y, ctx, init_state, init_memory, xc_mask_2]
        outs = [next_probs, next_sample, next_state, next_memory]
    else:
        inps = [y, ctx, init_state, xc_mask_2]
        outs = [next_probs, next_sample, next_state]
...
def gen_sample_2(tparams, f_init_2, f_next_2, xc, xc_mask, xc_mask_2, xc_mask_3, options, trng=None, maxlen=30):
    ...
    # get initial state of decoder rnn and encoder context
    ret = f_init_2(xc, xc_mask, xc_mask_2, xc_mask_3) 
    if options['decoder'].startswith('lstm'):
        next_state, ctx, next_memory = ret[0], ret[1], ret[2]
    else:
        next_state, ctx = ret[0], ret[1]
    next_w = -1 * numpy.ones((xc.shape[1],)).astype('int64')  # bos indicator

    for ii in xrange(maxlen):
        if options['decoder'].startswith('lstm'):
            inps = [next_w, ctx, next_state, next_memory, xc_mask_2]
        else:
            inps = [next_w, ctx, next_state, xc_mask_2]
        ret = f_next_2(*inps)
        if options['decoder'].startswith('lstm'):
            next_p, next_w, next_state, next_memory = ret[0], ret[1], ret[2], ret[3]
        else:
...
def pred_probs(f_log_probs, prepare_data, options, iterator, verbose=False):
    ...
    for y, xc in iterator:
        n_done += len(xc)

        y, y_mask, xc, xc_mask, lengths_y, lengths_xc, xc_mask_2, xc_mask_3 = prepare_data(y, xc,
                                            n_words_src=options['n_words_src'],
                                            n_words=options['n_words'])

        pprobs = f_log_probs(y, y_mask, xc, xc_mask, xc_mask_2, xc_mask_3)
        for pp in pprobs:
            probs.append(pp)
...
def train(rng=123,
    ...
    inps = [y, y_mask, xc, xc_mask, xc_mask_2, xc_mask_3]
    ...
    if alpha_c > 0. and not model_options['decoder'].endswith('simple'):
        ...
        alpha_reg = alpha_c * (
            (tensor.cast(y_mask.sum(0)//xc_mask_2.sum(0), 'float32')[:, None] -
    ...
    for eidx in xrange(max_epochs):
        ...
        for y, xc in train:
            ...
            y, y_mask, xc, xc_mask, lengths_y, lengths_xc, xc_mask_2, xc_mask_3 = prepare_data(y, xc, maxlen=maxlen,
            ...
            cost = f_grad_shared(y, y_mask, xc, xc_mask, xc_mask_2, xc_mask_3)
