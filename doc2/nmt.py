...
layers = {'ff': ('param_init_fflayer', 'fflayer'),
          ...
          'gru_ln_cond_fall1': ('param_init_gru_ln_cond_fall1', 'gru_ln_cond_fall1_layer'),
...
def init_params(options):
    ...
    if options['kwargs'].get('context_birnn', False):
        if options['decoder'] != 'lstm_cond_v6':
            ...
            params = get_layer(options['encoder'])[0](options, params,
                                                      ...
                                                      nin=options['dim_word'],
                                                      dim=options['dim'], rng=rng)
    elif options['decoder'] == 'lstm_cond_v6':
    ...
    params = get_layer('ff')[0](options, params, prefix='ff_logit_ctx',
                                ...
                                ortho=False, rng=rng)
    if options['kwargs'].get('use_logit_sc', True):
        if options['decoder'] != 'lstm_cond_v6':
            ...
            params = get_layer('ff')[0](options, params, prefix='ff_logit_sc',
                                nin=ctxdim, nout=options['dim_word'],
                                ortho=False, rng=rng)
    if options['kwargs'].get('very_late', False):
        if options['decoder'] != 'lstm_cond_v6':
            params = get_layer('ff')[0](options, params, prefix='ff_very_late',
                                        nin=options['dim_word'], nout=options['dim_word'],
                                        ortho=False, rng=rng)
        else:
            params = get_layer('ff')[0](options, params, prefix='ff_very_late',
...
def build_model(tparams, options):
    ...
    if options['kwargs'].get('context_birnn', False):
        ...
        context_emb = get_layer('ff')[1](tparams, context_emb, options,
                                    prefix='v6_context_emb', activ='linear')
    ...
    if options['kwargs'].get('very_late', False):
        vl_mask = get_layer('ff')[1](tparams, tsc, options,
                                   prefix='ff_very_late', activ='linear')
        logit *= tensor.nnet.sigmoid(vl_mask)
...
# Conditional LSTM layer with Attention
def param_init_lstm_cond_v6(options, params, prefix='lstm_cond_v6',
                        nin=None, dim=None, dimctx=None, rng=None):
    if nin is None:
        nin = options['dim']
    if dim is None:
...
def lstm_cond_v6_layer(tparams, state_below, options, prefix='lstm_cond_v6',
                   mask=None, context=None, one_step=False, init_state=None, init_memory=None,
                   context_mask=None, **kwargs):
...
# Conditional LSTM layer with Attention
def param_init_lstm_cond_fall0(options, params, prefix='lstm_cond_fall0',
                        nin=None, dim=None, dimctx=None, rng=None):
    if nin is None:
        nin = options['dim']
    if dim is None:
    ...
    Wc_sc = norm_weight(nin, dim*4, rng=rng)
...
def lstm_cond_fall0_layer(tparams, state_below, options, prefix='lstm_cond_fall0',
                   mask=None, context=None, one_step=False, init_state=None, init_memory=None,
                   context_mask=None, **kwargs):
    ...
    # step function to be used by scan
    # arguments    | sequences      |  outputs-info   | non-seqs ...
    def _step_slice(m_, x_, xc_, xsc_,
                    h_, ctx_, alpha_, c_, tsc, sc_alpha_,
                    pctx_, cc_, sc_pctx_, sc_cc_,
                    U, Wc, Wc_sc, Wd_att, Wd_sc_att, U_att, U_sc_att, c_tt, c_sc_tt):
...
# Conditional GRU layer with Attention
def param_init_gru_ln_cond_fall1(options, params, prefix='gru_ln_cond_fall1',
                        nin=None, dim=None, dimctx=None, rng=None):
    if nin is None:
        nin = options['dim']
    if dim is None:
    ...
    params[_p(prefix, 'b5')] = scale_add * numpy.ones((1*dimctx)).astype(floatX)
    params[_p(prefix, 'b6')] = scale_add * numpy.ones((1*nin)).astype(floatX)
    ...
    params[_p(prefix, 's5')] = scale_mul * numpy.ones((1*dimctx)).astype(floatX)
    params[_p(prefix, 's6')] = scale_mul * numpy.ones((1*nin)).astype(floatX)
...
def gru_ln_cond_fall1_layer(tparams, state_below, options, prefix='gru_ln_cond_fall1',
                   mask=None, context=None, one_step=False, init_state=None,
                   context_mask=None, **kwargs):
    ...
    def _step_slice(m_, x_, xx_, xc_, xsc_,
        ...
        # Now do the same for the neighbouring sentences
        sc_pstate_ = tensor.dot(h_, Wd_sc_att)
        sc_pstate_ = ln(sc_pstate_, b6, s6)
...
def train(rng=123,
    ...
    if model_options['kwargs'].get('pretrained_model', None) is not None:
        ...
        print numpy.shape(params['decoder_Wd_sc_att']), numpy.shape(old_params['decoder_Wd_att'])
        print numpy.shape(params['decoder_U_sc_att']), numpy.shape(old_params['decoder_U_att'])
        print numpy.shape(params['decoder_c_sc_tt']), numpy.shape(old_params['decoder_c_tt'])
        print numpy.shape(params['decoder_Wc_sc_att']), numpy.shape(old_params['decoder_Wc_att'])
        print numpy.shape(params['decoder_b_sc_att']), numpy.shape(old_params['decoder_b_att'])
        print numpy.shape(params['decoder_Wi_sc_att']), numpy.shape(old_params['decoder_Wi_att'])
