...
def init_params(options):
    ...
    params = get_layer('ff')[0](options, params, prefix='ff_logit_sc',
                            ...
                            ortho=False, rng=rng)
    params = get_layer('ff')[0](options, params, prefix='sc_gate',
...
def build_model(tparams, options):
    ...
    gate_sc = get_layer('ff')[1](tparams, proj_h, options,
                                   prefix='sc_gate', activ='tensor.nnet.sigmoid')
    logit_sc = get_layer('ff')[1](tparams, tsc, options,
                                   prefix='ff_logit_sc', activ='linear')
    logit_sc = tensor.addbroadcast(gate_sc, 2) * logit_sc
...
def build_sampler(tparams, options, trng, use_noise=None):
    ...
    logit_ctx = get_layer('ff')[1](tparams, ctxs, options,
                                   prefix='ff_logit_ctx', activ='linear')
    gate_sc = get_layer('ff')[1](tparams, next_state, options,
                                   prefix='sc_gate', activ='tensor.nnet.sigmoid')
    logit_sc = get_layer('ff')[1](tparams, tsc, options,
                                   prefix='ff_logit_sc', activ='linear')
    logit_sc = tensor.addbroadcast(gate_sc, 1) * logit_sc
